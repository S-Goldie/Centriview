...
#Discussion of fraction populations
...
+ \frac{\omega_i^2 t_i}{\omega_i^2 t_i + \omega_{i-1}^2 t_{i-1}} \frac{R_1}{R_2} e^{S \omega_{i-1}^2 t_{i-1}} = 1
''')
st.markdown(r'Solving this equation requires the approximation that $\omega_{𝑖}^{2} 𝑡_{𝑖} \approx \omega_{𝑖−1}^{2} 𝑡_{𝑖−1}$, i.e. that the centrifugation ' \
'conditions change vary little between steps in the cascade. This is the case for centrifugation scenarios that ' \
'target a significant narrowing of the initial distribution. Applying this approximation yields:')
st.latex(r'(LN)_{MP} \approx \frac{10.5 \, \eta \, \sqrt{k_{lw}}}{\omega_i^2 \, t d_{0} \Delta \rho_{NS}} \ln\left(\frac{2R_2}{R_2 + R_1}\right) \approx \langle LN \rangle')
st.markdown(r'It is then a trivial rearrangement to define $\omega^{2}t$ in terms of $\langle LN \rangle$.')
